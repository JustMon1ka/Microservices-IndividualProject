document.addEventListener('DOMContentLoaded', function() {
    ...
    // 发起请求，获取火车票信息
    fetch(`http://localhost:9999/get_trains?from_station=${toStation}&to_station=${fromStation}&departure=${backDate}`)
...
document.getElementById('skip-btn').addEventListener('click', function() {
    window.location.href = `../make_plan/main_screen/index.html?departure=${departureDate}&return=${backDate}`;  // 跳转到下一个页面
});
...
document.getElementById('confirm-train-btn').addEventListener('click', function() {
    ...
    if (selectedTrain) {
        ...
        const fromStation = selectedTrain.getAttribute('data-from');
        ...
        const trainEvent = {
            ...
            "date": backDate,         // 出发日期
            "title": trainNo,              // 车次号
            "details": `${fromStation} 到 ${_toStation}`  // 出发站和到达站
        };
        ...
        // 跳转到日历页面，并带上日期参数
        window.location.href = `../make_plan/main_screen/index.html?departure=${departureDate}&return=${backDate}&toStation=${toStation}`;
    } else {
        alert('请选择一个车次');
    }
});
